class Strategy(StrategyBase):
    ...
    def handle_data(self):  # K线推送、Tick推送、固定时间间隔、指定时刻，这4种事件会触发handle_data()函数
        self.action_7337060685519264214_invoke()  # 执行路径1

    def action_7337060685519264214(self):
        ...
        if self.recent_high > 0 and self.recent_high > crt_price * 1.02 and mcb > 0:
                add_amount = 0
        ...
        if self.recent_low > 0 and self.recent_low < crt_price * 0.97 and sell_amount > 2 and mqts > 0:
    ...
    def action_7337060685519264214_invoke(self):  # 自编码动作
        self.action_7337060685519264214()
